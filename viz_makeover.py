...
def get_plot(res_df):
    ...
    fig = px.scatter(data_frame=res_df, x="day", y="num_rats", render_mode="auto", color="Status", width=1700, symbol=None,
                     height=700,
    ...
    cols = ["#075216", "#00010a", "#fcf00a"]
    for i, id in enumerate(tr_line):
        fig.data[id].update(line_width=3, line_dash="longdashdot", line_color=cols[i])

    fig.update_layout(title_x=0.47, title_y=0.91, margin_r=0, margin_l=0, font_size=15, font_family="Arial Black")
    return fig
...
obs = """
* Ratio of **Closed** (blue points) to **Pending** (red points) complaints was almost 1:1 from 2010 to 2011. And greatly improved thereafter, hence showing **DSNY's promptness to address rodent infestations**
